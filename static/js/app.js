...
(() => {
  ...
  function openBulkUpload() {
    const sections = (typeof NAV_ORDER !== 'undefined' ? NAV_ORDER : [])
      .filter((n) => n.type === 'item' && n.key !== 'dashboard')
      .map((n) => n.key);
    ...
    overlay.innerHTML = `
      <div class="modal" style="max-width:560px;">
        ...
        <div class="modal-body" style="padding:20px;">
          <p style="margin:0 0 14px; font-size:13px; color:var(--gray-600); line-height:1.6;">
            Upload a <strong>.csv</strong> or <strong>.xlsx</strong> file. The section is detected from the
            <strong>end of the file name</strong> — e.g. <code>jan_students.csv</code> goes to
            <strong>Students</strong>, <code>list_trainers.xlsx</code> goes to <strong>Trainers</strong>.
            Column headers should match the field names (friendly names like “Full Name”, “Email ID”,
            “Mobile No”, “Company”, or a Course/Project name are understood too).
          </p>
          <label id="drop-zone" style="display:flex; flex-direction:column; align-items:center; justify-content:center;
                 gap:8px; padding:26px; border:2px dashed var(--gray-300); border-radius:10px; cursor:pointer;
                 background:var(--gray-50); text-align:center;">
            <svg viewBox="0 0 24 24" width="34" height="34" fill="none" stroke="var(--brand-500)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
            <span style="font-size:13.5px; color:var(--gray-700); font-weight:600;">Click to choose a file</span>
            <span id="bulk-fname" style="font-size:12px; color:var(--gray-500);">No file selected</span>
            <input type="file" id="bulk-file" hidden accept=".csv,.xlsx,.xlsm" />
          </label>
          <div id="bulk-result" style="margin-top:14px;"></div>
          <div style="margin-top:18px; display:flex; justify-content:flex-end; gap:8px;">
            <button class="btn btn-ghost" id="bulk-cancel">Close</button>
            <button class="btn btn-primary" id="bulk-submit" disabled>Upload</button>
          </div>
          <details style="margin-top:14px;">
            <summary style="font-size:12px; color:var(--gray-500); cursor:pointer;">Supported sections</summary>
            <div style="font-size:11.5px; color:var(--gray-500); margin-top:6px; line-height:1.7;">
              ${sections.map((s) => `<code>…_${s}</code>`).join(' · ')}
            </div>
          </details>
        </div>
      </div>`;
    ...
    const fnameEl = overlay.querySelector('#bulk-fname');
    const submitBtn = overlay.querySelector('#bulk-submit');
    ...
    ['dragover', 'dragenter'].forEach((ev) => dropZone.addEventListener(ev, (e) => {
      e.preventDefault(); dropZone.style.borderColor = 'var(--brand-500)';
    }));
    ['dragleave', 'drop'].forEach((ev) => dropZone.addEventListener(ev, (e) => {
      e.preventDefault(); dropZone.style.borderColor = 'var(--gray-300)';
    }));
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          const hint = data.known_sections ? `<div style="margin-top:6px; font-size:11px;">Try ending the name with one of: ${data.known_sections.join(', ')}</div>` : '';
          resultEl.innerHTML = `<div class="badge badge-danger" style="white-space:normal; display:block; padding:10px;">${UI.escapeHtml(data.error || 'Upload failed')}${hint}</div>`;
        } else {
          const errs = (data.errors && data.errors.length)
            ? `<details style="margin-top:8px;"><summary style="font-size:12px; cursor:pointer; color:var(--warning);">${data.errors.length} row note(s)</summary>
               <ul style="margin:6px 0 0; padding-left:18px; font-size:11.5px; color:var(--gray-600);">${data.errors.map((e) => `<li>${UI.escapeHtml(e)}</li>`).join('')}</ul></details>`
            : '';
          resultEl.innerHTML = `
            <div style="background:var(--success-bg); color:var(--success); padding:12px; border-radius:8px; font-size:13px;">
              ✓ Imported into <strong>${UI.escapeHtml(data.entity)}</strong>:
              <strong>${data.inserted}</strong> added${data.skipped ? `, ${data.skipped} skipped` : ''} (of ${data.total} rows).
            </div>${errs}
            <div style="margin-top:10px; font-size:12px; color:var(--gray-500);">Opening the ${UI.escapeHtml(data.entity)} section…</div>`;
          UI.toast(`Added ${data.inserted} record(s) to ${data.entity}`, 'success');
          setTimeout(() => { close(); window.location.hash = '#' + data.entity; }, 1200);
        }
      } catch (err) {
        resultEl.innerHTML = `<div class="badge badge-danger">${UI.escapeHtml(err.message)}</div>`;
      } finally {
  ...
  function wireChrome() {
    ...
    const bulkBtn = document.getElementById('bulk-upload-action');
    if (bulkBtn) bulkBtn.addEventListener('click', () => openBulkUpload());
