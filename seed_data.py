...
def seed(conn):
    ...
    managers = ["Ravi Teja", "Suresh Babu", "Anita Desai", "Mohan Rao", "Kavitha Nair"]
    internships = []
    for sid in random.sample(range(1, 61), 18):
        ...
        payment_type = "Paid" if paid else "Unpaid"
        stipend = random.choice([5000, 8000, 10000, 12000, 15000]) if paid else 0
        visitor_card = f"VC-{random.randint(1000, 9999)}"
        manager = random.choice(managers)
        internships.append((sid, comp, role, start, end, status, payment_type, stipend, visitor_card, manager))
    cur.executemany(
        "INSERT INTO internships (student_id,company_name,role,start_date,end_date,status,payment_type,stipend,visitor_card_id,reporting_manager) VALUES (?,?,?,?,?,?,?,?,?,?)",
        internships,
    )
